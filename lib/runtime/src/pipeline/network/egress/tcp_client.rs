...
impl TcpConnection {
    ...
    /// Send an already-validated request via the submit queue.
    ///
...
mod tests {
    ...
    #[tokio::test]
    async fn test_chunked_writer_backpressure_preserves_legacy_frames() {
        use crate::pipeline::network::codec::{TcpRequestMessage, ZeroCopyTcpDecoder};

        let messages: Vec<_> = [0, 4095, 4096, 65536]
            .into_iter()
            .map(|len| TcpRequestMessage::new("test.endpoint".into(), Bytes::from(vec![42; len])))
            .collect();
        let mut expected = Vec::new();
        let mut write_buf = TcpWriteBuffer::new();
        for msg in &messages {
            expected.extend_from_slice(&msg.encode().unwrap());
            write_buf.push_frame(msg.clone().into_frame().unwrap());
        }

        // A tiny duplex buffer forces Pending and partial writes. Its scalar
        // AsyncWrite fallback also exercises writers without optimized writev.
        let (mut writer, mut reader) = tokio::io::duplex(31);
        let mut received = vec![0; expected.len()];
        let (written, read) = tokio::time::timeout(Duration::from_secs(5), async {
            tokio::join!(
                write_buf.write_all(&mut writer),
        ...
        // The unchanged ingress decoder accepts the stream produced by either
        // encoding path, including concatenated and empty-payload requests.
        let mut decoder = ZeroCopyTcpDecoder::new();
        let mut input = received.as_slice();
        for msg in messages {
            let decoded = decoder.read_message(&mut input).await.unwrap();
            assert_eq!(decoded.payload(), msg.payload.as_ref());
        }
    }

    #[tokio::test]
    async fn test_chunked_writer_disconnect_during_payload() {
        let mut write_buf = TcpWriteBuffer::new();
        write_buf.write(Bytes::from_static(b"header"));
        write_buf.write(Bytes::from(vec![42; 65536]));
        let (mut writer, mut reader) = tokio::io::duplex(31);
        let (result, ()) = tokio::time::timeout(Duration::from_secs(5), async {
            tokio::join!(write_buf.write_all(&mut writer), async move {
                reader.read_exact(&mut [0; 32]).await.unwrap();
                drop(reader);
            })
        })
        .await
